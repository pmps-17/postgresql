			...
			bool *foundPtr)
{
	...
	if (buf_id >= 0)
	{
		...
		*foundPtr = true;
		...
		RemoveFromLruList(buf, buf_state);
...
UnpinBuffer(BufferDesc *buf, bool fixOwner)
{
	...
	if (ref->refcount == 0)
	{
		...
		/*----------------- project 3 changes ------------------------------*/
		/*
		 * Adding the unpinned buffer to LRU linked list when shared reference count = 0.
		 */
		AddToLruList(buf, buf_state);
